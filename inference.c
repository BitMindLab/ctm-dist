...
double f_lambda(const gsl_vector * p, void * params)  //目标函数中所有带 lambda的项, p 是子变量
{
...
/*    for (i = 0; i < mod->k-1; i++)
    ...
    gsl_blas_ddot(p,((bundle *) params)->sum_phi, &term1);  //计算向量点乘 term1=p*sum_phi
...
int opt_lambda(llna_var_param * var, doc * doc, llna_model * mod)
{
    ...
    //显示参数值
        /*printf("mu=");
        for(i=0;i<mod->k-1;i++)
        	printf("%lf\t",vget(mod->mu,i));

        printf("\nlambda=");
        for(i=0;i<mod->k-1;i++)
        	printf("%lf\t",vget(var->lambda,i));

        printf("\nnu=");
        for(i=0;i<mod->k-1;i++)
        	printf("%lf\t",vget(var->nu,i));
    ...
    //共轭梯度法中主要的函数设置---注意下面三个函数f，df，fdf
...
/*    printf("final_lambda=");
    for (i = 0; i < mod->k-1; i++)
    {
        vset(var->lambda, i, vget(s->x, i));  //------更新lambda-----
        printf("%lf\t",vget(var->lambda,i));
    }
    printf("\n");*/
    vset(var->lambda, i, 0);
                     ...
                     llna_model * mod)
{
    ...
    do
    {
        ...
        opt_lambda(var, doc, mod);  //比较复杂
        //printf("function opt_lambda done\n");
        opt_zeta(var, doc, mod);
...
void update_expected_ss(llna_var_param* var, doc* d, llna_ss* ss)
{
    ...
    {
        vinc(ss->mu_ss, i, vget(var->lambda, i));  //gsl_vector_get
