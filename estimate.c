                 ...
                 short reset_var, double* converged_pct)
{
    ...
    gsl_vector* phi_sum;  //貌似可以融入到   llna_var_param* var 中
    ...
    *converged_pct = 0;
    ...
    for (i = 0; i < corpus->ndocs; i++)
    {
        printf("doc %5d   ", i);
        doc = corpus->docs[i];
        var = new_llna_var_param(doc.nterms, model->k);  //对变分参数var分配存储空间
        ...
        //lhood = 0.01;
        lhood = var_inference(var, &doc, model);  //---------很重要：更新变分参数, doc+model=>var-------
        update_expected_ss(var, &doc, ss);   //更新的ss,用于M步的参数估计
        total += lhood;
...
llna_model* em_initial_model(int k, corpus* corpus, char* start)
{
    ...
    if (strcmp(start, "rand")==0)
        model = random_init(k, corpus->nterms);  //这是随机初始化，0均值，单位阵方差，beta为随机值
    else if (strcmp(start, "seed")==0)
        model = corpus_init(k, corpus);    //这是随机初始化
    else
...
void em(char* dataset, int k, char* start, char* dir)
{
    ...
    ss = new_llna_ss(model);    //申请ss内部变量存储空间空间
...
int main(int argc, char* argv[])
{
    if (argc > 1)
    {
        if (strcmp(argv[1], "est")==0)
        {
            ...
            inference("train_for_lda_150.data","000","sfds");
            return(0);
        }
